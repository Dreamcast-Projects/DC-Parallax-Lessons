...
int main(int argc, char **argv) {
	...
	while(1) {
	    ...
	    plx_mat3d_rotate(rtri, 0.0f, 1.0f, 0.0f);  // Rotate the triangle angle 'rtri' on the Y axis
	    ...
	    plx_mat3d_rotate(rquad, 1.0f, 0.0f, 0.0f);  // Rotate the quad angle 'rquad' on the X axis
		...
		rquad -= 2.0f; // Increases the quad's rotation angle
		rtri += 2.0f;  // Increases the triangle's rotation angle
    }
